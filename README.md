...
Personal AI coding agent configuration for a freelance frontend creative developer. Claude Code native format with a built-in conversion step for Cursor, Windsurf, Copilot, and Aider.

## Quick Start

### Option A: Global Install (git clone)
...
Creates three symlinks in `~/.claude/`:
...
### Option B: Project Init (npm/bunx)
...
# Claude + Cursor
bunx @nickbevers/claude-rules init claude cursor

# All agents
bunx @nickbevers/claude-rules init all
```

Copies `CLAUDE.md`, `rules/`, and `skills/` into your project. For non-Claude agents, generates their config by stripping frontmatter and concatenating rules into the format each agent expects.

### Converting for Other Agents

```bash
# From a project that already has rules/
claude-rules convert cursor       # -> .cursorrules
claude-rules convert windsurf     # -> .windsurfrules
claude-rules convert copilot      # -> .github/copilot-instructions.md
claude-rules convert aider        # -> .aider.conf.yml + CONVENTIONS.md
claude-rules convert all          # All of the above
```

The conversion reads `rules/*.md` and `skills/*/SKILL.md`, strips YAML frontmatter, and concatenates into the flat format each agent expects. Agent-specific guidance (e.g., "Cursor can't spawn subagents, use sequential sparring") is appended automatically.
...
- Hard constraints Claude gets wrong without them (OKLCH colors, animation performance, no icons)
...
The `convert` command strips YAML `paths:` frontmatter and concatenates all rules into the flat format each agent expects. Agent-specific extras (e.g., "Cursor can't spawn subagents") are embedded in the CLI and appended automatically.
